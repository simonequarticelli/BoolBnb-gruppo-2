{{-- inizio footer --}}
<footer>
  <div class="container footer-section">
      <div class="col-lg-3 col-sm-12">
        <h6>Airbnb</h6>
        <ul>
          <li><a href="#">Opportunità di lavoro</a></li>
          <li><a href="#">News</a></li>
          <li><a href="#">Condizioni</a></li>
          <li><a href="#">Aiuto</a></li>
          <li><a href="#">Diversità e trasparenza</a></li>
          <li><a href="#">Informazioni di contatto</a></li>
        </ul>
      </div>


      <div class="col-lg-3 col-sm-12">
        <h6>Scopri</h6>
        <ul>
          <li><a href="#">Opportunità di lavoro</a></li>
          <li><a href="#">News</a></li>
          <li><a href="#">Condizioni</a></li>
          <li><a href="#">Aiuto</a></li>
          <li><a href="#">Diversità e trasparenza</a></li>
          <li><a href="#">Informazioni di contatto</a></li>
        </ul>
      </div>

      <div class="col-lg-3 col-sm-12">
        <h6>Ospita</h6>
        <ul>
          <li><a href="#">Opportunità di lavoro</a></li>
          <li><a href="#">News</a></li>
          <li><a href="#">Condizioni</a></li>
          <li><a href="#">Aiuto</a></li>
          <li><a href="#">Diversità e trasparenza</a></li>
          <li><a href="#">Informazioni di contatto</a></li>
        </ul>
      </div>

      <div class="col-lg-3 col-sm-12">
        <ul>
          <li><a href="#">Termini</a></li>
          <li><a href="#">Privacy</a></li>
          <li><a href="#">Mappa del sito</a></li>
        </ul>
      </div>
    </div>
  {{-- parte inferiore footer --}}
  <div class="container sub-footer">
    <div class="col-lg-6 col-sm-12 sub-footer-left">
      <div class="footer-logo mr-3">
        <img src="{{asset('images/footer-logo.png')}}" alt="">
      </div>
      <div class="copy-logo">
        <span>© 2019 BoolBnb, Inc. Tutti i diritti riservati.</span>
      </div>
    </div>
    <div class="col-lg-6 col-sm-12 sub-footer-right">
     <a class="btn btn-light mr-3"  href="#">Italiano</a>
     <a class="btn btn-light "href="#">EUR-€</a>
    </div>

  </div>
</footer>
